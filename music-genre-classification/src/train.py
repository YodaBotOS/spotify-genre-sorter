...
def main():
    ...
    for epoch in range(EPOCH_NUM):
        inp_train, out_train = Variable(torch.from_numpy(x_train)).float().cpu(), Variable(
            torch.from_numpy(y_train)).long().cpu()
        inp_valid, out_valid = Variable(torch.from_numpy(x_valid)).float().cpu(), Variable(
            torch.from_numpy(y_valid)).long().cpu()
        # ------------------------------------------------------------------------------------------------- #
        ...
        for i in range(0, TRAIN_SIZE, BATCH_SIZE):
            ...
            train_loss += loss_train_batch.data.cpu().numpy().item()
        ...
        for i in range(0, TRAIN_SIZE, BATCH_SIZE):
            ...
            train_sum += (indices_train == out_train[i:i + BATCH_SIZE]).sum().data.cpu().numpy().item()
        ...
        for i in range(0, VALID_SIZE, BATCH_SIZE):
            ...
            valid_loss += loss_valid_batch.data.cpu().numpy().item()
        ...
        for i in range(0, VALID_SIZE, BATCH_SIZE):
            ...
            valid_sum += (indices_valid == out_valid[i:i + BATCH_SIZE]).sum().data.cpu().numpy().item()
    ...
    # ------------------------------------------------------------------------------------------------- #
    inp_test, out_test = Variable(torch.from_numpy(x_test)).float().cpu(), Variable(
        torch.from_numpy(y_test)).long().cpu()
    test_sum = 0
    for i in range(0, TEST_SIZE, BATCH_SIZE):
        pred_test = net(inp_test[i:i + BATCH_SIZE])
        indices_test = pred_test.max(1)[1]
        test_sum += (indices_test == out_test[i:i + BATCH_SIZE]).sum().data.cpu().numpy().item()
